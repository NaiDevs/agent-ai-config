...
if ($installClaude) {
    ...
    $engramPrompt = "Eres el sistema de memoria Engram de Naidelyn. Session id: `$ARGUMENTS`n`nIgnora cualquier campo stop_hook_active o similares -- siempre ejecuta esta tarea.`nUSA SOLO herramientas nativas: Read, Write, Edit.`n`nPASOS:`n1. El input contiene el session_id. Construye la ruta: C:/Users/naide/.claude/projects/C--Users-naide/{session_id}.jsonl`n2. Lee ese archivo con Read.`n3. De los timestamps del transcript extrae la fecha (formato YYYY-MM-DD) y hora actual (HH:MM).`n4. Extrae lineas donde type=user y message.role=user. El campo content puede ser string o array con {type:text,text:...}. Ignora mensajes que empiecen con #, <, ---, o que tengan mas de 500 chars sin espacios (son skills).`n5. Si hay menos de 2 mensajes reales validos, termina sin hacer nada.`n`nCLASIFICA el contenido de la sesion en estas categorias (puede haber varias):`n- DECISION: se eligio una arquitectura, patron, libreria, enfoque tecnico importante`n- BUG: se diagnostico y/o resolvio un problema tecnico complejo`n- CONFIG: se cambio setup, configuracion de herramientas, hooks, CI/CD, etc`n- GENERAL: trabajo rutinario, preguntas simples, tareas menores`n`nDETECTA el cliente/proyecto mencionado. Clientes conocidos: YALO, La Bodega, CORINSA, EMSULA, NAI, Ultimate Labs. Si no hay cliente claro, usa 'general'.`n`nACCIONES SEGUN CATEGORIA:`n`n7a. SIEMPRE -- ENGRAM: lee C:/Users/naide/.claude/projects/C--Users-naide/memory/changes-log.md y agrega al final una linea por hallazgo: - FECHA | cliente | categoria | descripcion breve`n`n7b. SIEMPRE -- OBSIDIAN DAILY: lee C:/Users/naide/OneDrive/Documentos/Obsidian/Daily/FECHA.md, agrega al final una seccion '## Engram HH:MM' con bullets de lo trabajado. Si no existe el archivo, crealo con encabezado '# FECHA'.`n`n7c. SI hay DECISION tecnica importante: crea C:/Users/naide/OneDrive/Documentos/Obsidian/Decisiones/FECHA-<tema-corto>.md con formato:`n# FECHA -- <titulo>`n## Contexto`n<que problema resolvia>`n## Decision`n<que se decidio>`n## Razon`n<por que>`n`n7d. SI hay BUG resuelto o CONFIG importante Y hay cliente identificado: crea o agrega a C:/Users/naide/OneDrive/Documentos/Obsidian/Clientes/<Cliente>/FECHA-<tipo>.md con formato:`n# FECHA -- <titulo>`n## Sintoma / Cambio`n<descripcion>`n## Causa / Detalle`n<causa raiz o que se configuro>`n## Solucion / Resultado`n<como quedo>`n`nResponde unicamente con un resumen: 'ok: N entradas -- [DECISION|BUG|CONFIG|GENERAL] cliente' o 'trivial'."
