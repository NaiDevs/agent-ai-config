...
- [Sync catálogos SPC/Delasa → YALO](project-spc-delasa-sync.md) — fx-sync-info (Lambda upsert genérico) + spc-delasa (orquestador); clientes/vendedores/pagos/categoría/territorio/impuestos/bodegas/productos+precios HECHOS (compila, sin probar); PENDIENTE bloqueantes (cód AUTH org, establecimiento real, deploy fx-sync-info, verificar nombres SPC, merges) + escalas v2 + correlativo pedido
- [Editar pedido Yalo Vendo duplica](decision-yalovendo-edicion-pedidos-duplicados.md) — causa raíz en sync offline (orders = OrderCreateMinimalDto sin id → siempre CreateMinimalAsync); PATCH online y monitor están OK
- [Eliminar pedido sigue visible en motorista](decision-yalovendo-entrego-eliminar-pedido-sync.md) — sync Entrego no comunicaba bajas; fix Parte 1 (knownOrderIds/removedOrderIds) hecho; Parte 2 desasignar/reasignar en BO API pendiente
